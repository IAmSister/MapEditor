...
//    private Brush eraserBrush;
    ...
    private void createMenuBar() {
        ...
        JMenuItem saveAsImage = createMenuItem("保存为图片",//Resources.getString("menu.file.image"), 
        		null,
        		"保存地图为图片格式",//Resources.getString("menu.file.image.tooltip"), 
        		"control shift I");
        JMenuItem close = new TMenuItem(new CloseMapAction(this, saveAction));
        
        recentMenu = new JMenu("最近打开的文件");//Resources.getString("menu.file.recent"));
        ...
        JMenu fileMenu = new JMenu("文件");//Resources.getString("menu.file"));
//        fileMenu.add(new TMenuItem(new SetBackAction(this, saveAction)));
        ...
        JMenu editMenu = new JMenu("编辑");//Resources.getString("menu.edit"));
...
//        editMenu.addSeparator();
        editMenu.add(createMenuItem("设置",//Resources.getString("menu.edit.preferences"),
                    null, 
                    "设置属性",//Resources.getString("menu.edit.preferences.tooltip"), 
                    null));
        editMenu.add(createMenuItem("刷子",//Resources.getString("menu.edit.brush"), 
        		null,
        			"设置刷子属性",//Resources.getString("menu.edit.brush.tooltip"),
                    "control B"));
        ...
        JMenu mapMenu = new JMenu("地图");//Resources.getString("menu.map"));
//        mapMenu.add(createMenuItem(Resources.getString("menu.map.resize"), null,
//        		Resources.getString("menu.map.resize.tooltip")));
        mapMenu.add(createMenuItem("搜索",//Resources.getString("menu.map.search"), 
        		null,
        		Resources.getString("menu.map.search.tooltip")));
        mapMenu.addSeparator();
        mapMenu.add(createMenuItem("属性",//Resources.getString("menu.map.properties"), 
        		null,
        ...
        JMenu tilesetMenu = new JMenu("素材集");//Resources.getString("menu.tilesets"));
        tilesetMenu.add(createMenuItem("创建新素材集",//Resources.getString("menu.tilesets.new"), 
        		null,
        		Resources.getString("menu.tilesets.new.tooltip")));
        tilesetMenu.add(createMenuItem("导入素材集",//Resources.getString("menu.tilesets.import"), 
        		null,
        		Resources.getString("menu.tilesets.import.tooltip")));
        tilesetMenu.addSeparator();
        tilesetMenu.add(createMenuItem("素材集管理",//Resources.getString("menu.tilesets.manager"), 
        		null,
        ...
        */
        ...
        gridMenuItem = new JCheckBoxMenuItem("显示网格");//Resources.getString("menu.view.grid"));
        gridMenuItem.addActionListener(this);
        gridMenuItem.setToolTipText("显示网格");//Resources.getString("menu.view.grid.tooltip"));
        gridMenuItem.setAccelerator(KeyStroke.getKeyStroke("control G"));

        cursorMenuItem = new JCheckBoxMenuItem("高亮鼠标指针");//Resources.getString("menu.view.cursor"));
        cursorMenuItem.setSelected(prefs.getBoolean("cursorhighlight", true));
        cursorMenuItem.addActionListener(this);
        cursorMenuItem.setToolTipText("高亮显示鼠标所在网格");//Resources.getString("menu.view.cursor.tooltip"));
        ...
        coordinatesMenuItem = new JCheckBoxMenuItem("显示坐标");//Resources.getString("menu.view.coordinates"));
        coordinatesMenuItem.addActionListener(this);
        coordinatesMenuItem.setToolTipText("显示网格坐标");//Resources.getString("menu.view.coordinates.tooltip"));

        JMenu viewMenu = new JMenu("视图");//Resources.getString("menu.view"));
...
//        viewMenu.addSeparator();
        ...
        JMenu helpMenu = new JMenu("帮助");//Resources.getString("menu.help"));
//        helpMenu.add(createMenuItem(Resources.getString("menu.help.plugins"), null,
//        		Resources.getString("menu.help.plugins.tooltip")));
        helpMenu.add(createMenuItem("关于",//Resources.getString("menu.help.about"), 
...
//        toolBar.add(new TButton(zoomOutAction));
    ...
    private void handleEvent(ActionEvent event) {
        ...
        if (command.equals("刷子")){//Resources.getString("menu.edit.brush"))) {
            BrushDialog bd = new BrushDialog(this, appFrame, currentBrush);
            bd.setVisible(true);
        } else if (command.equals("创建新素材集")){//Resources.getString("menu.tilesets.new"))) {
            if (currentMap != null) {
        ...
        } else if (command.equals("导入素材集")){//Resources.getString("menu.tilesets.import"))) {
            if (currentMap != null) {
        ...
        } else if (command.equals("素材集管理")){//Resources.getString("menu.tilesets.manager"))) {
            if (currentMap != null) {
                TilesetManager manager = new TilesetManager(appFrame, currentMap);
                manager.setVisible(true);
            }
        } else if (command.equals("保存为图片")){//Resources.getString("menu.file.image"))) {
            if (currentMap != null) {
                saveMapImage(null);
            }
        } else if (command.equals("属性")){//Resources.getString("menu.map.properties"))) {
            PropertiesDialog pd = new PropertiesDialog(appFrame,
                    currentMap.getProperties());
            pd.setTitle("地图属性");//Resources.getString("dialog.properties.map.title"));
            pd.getProps();
        ...
        } else if (command.equals("显示网格")){//Resources.getString("menu.view.grid"))) {
            // Toggle grid
        ...
        } else if (command.equals("显示坐标")){//Resources.getString("menu.view.coordinates"))) {
            // Toggle coordinates
            mapView.toggleMode(MapView.PF_COORDINATES);
        } else if (command.equals("高亮鼠标指针")){//Resources.getString("menu.view.cursor"))) {
            ...
            rd.setVisible(true);
        }  else if (command.equals("搜索")){//Resources.getString("menu.map.search"))) {
            SearchDialog sd = new SearchDialog(appFrame, currentMap);
            sd.setVisible(true);
        } else if (command.equals("关于")){//Resources.getString("menu.help.about"))) {
            if (aboutDialog == null) {
        ...
        } else if (command.equals("设置")){//Resources.getString("menu.edit.preferences"))) {
    ...
    private class ZoomInAction extends AbstractAction {
		...
		private static final long serialVersionUID = 1L;
		public ZoomInAction() {
            super("放大");//Resources.getString("action.zoom.in.name"));
            putValue(ACCELERATOR_KEY,
                    KeyStroke.getKeyStroke("control EQUALS"));
            putValue(SHORT_DESCRIPTION,
                     "放大显示");//Resources.getString("action.zoom.in.tooltip"));
    ...
    private class ZoomOutAction extends AbstractAction {
		...
		public ZoomOutAction() {
            super("缩小");//Resources.getString("action.zoom.out.name"));
            putValue(ACCELERATOR_KEY,
                    KeyStroke.getKeyStroke("control MINUS"));
            putValue(SHORT_DESCRIPTION,
                     "缩小显示");//Resources.getString("action.zoom.out.tooltip"));
    ...
    private class ZoomNormalAction extends AbstractAction {
		...
		public ZoomNormalAction() {
            super("正常");//Resources.getString("action.zoom.normal.name"));
            putValue(ACCELERATOR_KEY,
                    KeyStroke.getKeyStroke("control 0"));
            putValue(SHORT_DESCRIPTION,
                     "正常尺寸显示");//Resources.getString("action.zoom.normal.tooltip"));
        }
        public void actionPerformed(ActionEvent evt) {
            if (currentMap != null) {
                zoomInAction.setEnabled(true);
...
*/                }
    ...
    public void updateTitle() {
        String title = "猫扑地图编辑器";//Resources.getString("dialog.main.title");
        ...
        if (currentMap != null) {
            ...
            if (filename != null) {
                ...
                title += "地图";//Resources.getString("general.file.untitled");
            }
            if (unsavedChanges()) {
                title += "*";
            }
    ...
    public boolean loadMap(String file) {
        ...
        if (!exist.exists()) {
            JOptionPane.showMessageDialog(appFrame,
                    "文件不存在",//Resources.getString("general.file.notexists.message"),
                    "打开地图文件失败",//Resources.getString("dialog.openmap.error.title"),
        ...
        try {
            ...
            if (m != null) {
                ...
                JOptionPane.showMessageDialog(appFrame,
                        "加载地图失败",//Resources.getString("general.file.failed"),
                        "打开地图失败",//Resources.getString("dialog.openmap.error.title"),
                        JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(appFrame,
                    "加载 " + file + ": " +
                    e.getLocalizedMessage() + (e.getCause() != null ? " 失败\n原因: " +
